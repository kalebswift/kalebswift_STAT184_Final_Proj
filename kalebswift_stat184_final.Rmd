...
gs_data <- read.csv("8f79c865-4e73-43e3-a534-351fdca8f75d_Data.csv", header=TRUE)
```
...
```{r}
wdi_GDP <- wdi %>% filter(code=="NY.GDP.MKTP.CD") %>%
...
wdi_2 <- wdi_GDP %>% left_join(wdi_growth, by=c("country"="country", "yr"="yr")) %>% 
  left_join(wdi_life, by=c("country"="country", "yr"="yr")) %>% mutate(year = substring(yr, 2)) %>%
  select(country, year, `GDP (current US$)`, `GDP growth (annual %)`, `Life expectancy at birth, total (years)`)
...
gs_2 <- gs_contra %>%
  ...
  left_join(gs_labor, by=c("country"="country", "yr"="yr")) %>% mutate(year = substring(yr, 2)) %>%
...
wdigs <- wdi_2 %>% left_join(gs_2, by=c("country"="country", "yr"="yr"))
View(wdigs)
```
